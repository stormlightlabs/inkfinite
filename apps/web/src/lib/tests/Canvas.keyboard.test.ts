import { Store, type Action } from '@inkfinite/core';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { cleanup, render } from 'vitest-browser-svelte';

const actionHandlers: Array<(action: Action) => void> = [];
const coreMocks = vi.hoisted(() => ({ storeInstances: [] as Store[] }));

async function selectShapeAt(
...
async function waitForDocumentReady() {
	await vi.waitFor(() => {
		...
		expect(state.ui.currentPageId).toBe('page:1');
		expect(state.doc.shapes['shape:1']).toBeTruthy();
	});
}
...
vi.mock('$lib/persistence/repository', () => ({
	createDexieDocRepo: vi.fn(() => ({
		listBoards: async () => [
			{ id: 'board-1', name: 'Test Board', createdAt: 0, updatedAt: 0 }
		],
		...
		loadDoc: async () => ({
			pages: {
				'page:1': { id: 'page:1', name: 'Page 1', shapeIds: ['shape:1', 'shape:2'] }
			},
			shapes: {
				'shape:1': {
					id: 'shape:1',
...
const executeCommandSpy = vi.spyOn(Store.prototype, 'executeCommand');
...
describe('Canvas keyboard shortcuts', () => {
	beforeEach(() => {
		...
		coreMocks.storeInstances.length = 0;
	...
	it('should nudge selected shapes with arrow keys', async () => {
		...
		await vi.waitFor(() => {
			const calls = executeCommandSpy.mock.calls;
