...
describe('InputAdapter', () => {
	...
	describe('pointer events', () => {
		...
		it('should dispatch pointer up action', () => {
			...
			expect(actions[1].type).toBe('pointer-up');
			expect(actions[1]).toMatchObject({ screen: { x: 100, y: 200 }, button: 0 });
		});
		...
		it('finishes a captured pointer when it is released outside the canvas', () => {
			const setPointerCapture = vi.fn();
			const releasePointerCapture = vi.fn();
			vi.spyOn(canvas, 'setPointerCapture').mockImplementation(setPointerCapture);
			vi.spyOn(canvas, 'hasPointerCapture').mockReturnValue(true);
			vi.spyOn(canvas, 'releasePointerCapture').mockImplementation(releasePointerCapture);
			...
			expect(adapter.getPointerState()).toMatchObject({
				isDown: false,
				startScreen: null,
				startWorld: null
			});
		...
		it('clears an interaction when pointer capture is lost', () => {
			canvas.dispatchEvent(
				createPointerEvent('pointerdown', {
					clientX: 100,
					clientY: 100,
			...
			expect(adapter.getPointerState().isDown).toBe(false);
		});
		...
		it('should decode pointer buttons bitmask', () => {
			...
			expect(actions[0]).toMatchObject({
				buttons: { left: true, middle: true, right: false }
			});
		});
	});
	...
	describe('coordinate transformation', () => {
		it('uses current canvas bounds and viewport after a resize', () => {
			expect([1, 2]).toContain(window.devicePixelRatio);
			...
			resizedCanvas.dispatchEvent(
				createPointerEvent('pointermove', { clientX: 400, clientY: 300 })
			);
			expect(resizedActions.at(-1)).toMatchObject({
				screen: { x: 400, y: 300 },
				world: { x: 0, y: 0 }
			});
			...
			resizedCanvas.dispatchEvent(
				createPointerEvent('pointermove', { clientX: 300, clientY: 200 })
			);
			...
			resizedCanvas.dispatchEvent(
				createWheelEvent({ clientX: 150, clientY: 125, deltaY: -100 })
			);
	...
	describe('edge cases', () => {
		...
		it('should handle multiple pointer buttons simultaneously', () => {
			...
			expect(actions[0]).toMatchObject({
				button: 2,
				buttons: { left: true, middle: true, right: true }
			});
		});
