...
describe('MarkdownEditorController', () => {
	...
	describe('getLayout', () => {
		...
		it('should handle auto-computed height', () => {
			...
			const shape = EditorShapeRecord.createMarkdown(
				...
				100,
				200,
				{ md: '# Test', w: 300, fontSize: 16, fontFamily: 'sans-serif', color: '#000' },
				'shape1'
			);
	...
	describe('handleInput', () => {
		it('should update current value on input', () => {
			...
			const mockEvent = {
				currentTarget: { value: '# Hello World' } as HTMLTextAreaElement
			} as unknown as Event;
		...
		it('should do nothing if not editing', () => {
			const mockEvent = {
				currentTarget: { value: 'test' } as HTMLTextAreaElement
			} as unknown as Event;
	...
	describe('commit', () => {
		...
		it('should not update if value is unchanged', () => {
			...
			const normalizedInitialState = store.getState();

			controller.start('shape1');
			controller.commit();

			const finalState = store.getState();
			expect(finalState).toEqual(normalizedInitialState);
		});
