...
function createStatusTracker(overrides?: {
	...
	const options: Omit<DexieAdapterOptions, 'database'> = overrides?.options ?? {
		liveQueryFn: live?.factory
	};
...
describe('Dexie editor adapter', () => {
	...
	it('records errors from flush', async () => {
		...
		(repo.applyDocPatch as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
			new Error('failed')
		);
