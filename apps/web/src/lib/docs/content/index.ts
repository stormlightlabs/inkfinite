...
import { docSections, type Doc, type DocFrontmatter, type DocHeading } from './types';

type MarkdownModule = { default: Component; metadata?: unknown };

const markdownModules = import.meta.glob<MarkdownModule>('/src/content/docs/**/*.md', {
	eager: true
});
...
function requiredToc(value: Record<string, unknown>, source: string): DocHeading[] {
	...
	return value.toc.map((heading, index) => {
		...
		if (level !== 2 && level !== 3) {
			throw new Error(
				`Invalid frontmatter in ${source}: toc[${index}].level must be 2 or 3`
			);
		}
...
/** Validates frontmatter at build time so navigation cannot drift from page content. */
export function validateFrontmatter(value: unknown, source: string): DocFrontmatter {
	...
	return {
		...
		section: section as DocFrontmatter['section'],
...
/** All documentation pages in sidebar and reading order. */
