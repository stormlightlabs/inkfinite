...
function preferredSearch() {
	pruneSearchStates();
	const visible =
		searchStates.find((state) => isVisible(state.trigger)) ??
		searchStates.find((state) => isVisible(state.box));
...
function setSearchEnabled(state, enabled) {
	state.input.disabled = !enabled;
	state.submit.disabled = !enabled;
}

async function ensureSearchIndex(state) {
	try {
		const index = await getPagefind();
		await index.search('__inkfinite_search_probe__');
		state.indexAvailable = true;
		if (state.dialog.open) {
			setSearchEnabled(state, true);
			state.input.focus();
		}
	} catch {
		state.indexAvailable = false;
		setSearchEnabled(state, false);
		if (state.dialog.open) {
...
async function search(state, query, version) {
	...
	try {
		...
		const matches = settledMatches.flatMap((match) =>
		...
		for (const [index, match] of matches.entries()) {
			...
			excerpt.innerHTML = escapeHtml(match.excerpt ?? '').replace(
				/&lt;(mark|\/mark)&gt;/g,
				'<$1>'
			);
...
function focusReturnTarget(state) {
	...
	state.returnFocus = null;
	const fallback = searchStates
		.map((state) => state.trigger)
		.find((trigger) => isVisible(trigger));
...
function openSearch(state) {
	...
	activeSearch = state;
	void ensureSearchIndex(state);
	window.requestAnimationFrame(() => {
		if (!state.input.disabled) state.input.focus();
	});
}
...
function initializeSearchBox(box) {
	...
	const submit = box.querySelector('[data-search-submit]');
