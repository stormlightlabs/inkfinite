...
/** Serialized status returned by every Rust-owned desktop session command. */
...
/** Result returned after Rust creates or opens a desktop session. */
export type SessionOpened = { session_id: string; status: SessionStatus };
/** Result returned after Rust commits, undoes, or redoes a transaction. */
export type SessionCommit = { commit: CommitResult; status: SessionStatus };
/** Result returned after Rust persists a session. */
...
/** Persistent document repository backed by one Rust-owned Tauri session. */
 ...
 * memory until Rust returns a committed snapshot.
 */
export function createDesktopSessionRepo(
  fileOps: DesktopFileOps,
  options: { api?: SessionApi } = {},
): DesktopSessionRepo {
  const api = options.api ?? createSessionApi();
  ...
  async function openPath(path: string, boardName?: string): Promise<LoadedDoc> {
    ...
    const opened = await api.openDocument({ path, actor_id: actorId });
  ...
  async function applyDocPatch(boardId: string, patch: DocPatch): Promise<void> {
    ...
    const target = documentFromLoadedDoc(nextDoc, currentStatus.snapshot, actorId);
    ...
    // Desktop edits are persisted by the Rust service before the input event
  ...
  async function undo(): Promise<void> {
    if (!currentStatus) return;
    const result = await api.undo({ session_id: currentStatus.session_id, actor_id: actorId });
  ...
  async function redo(): Promise<void> {
    if (!currentStatus) return;
    const result = await api.redo({ session_id: currentStatus.session_id, actor_id: actorId });
    updateStatus(result.status);
    await saveCurrentSession();
  }
...
/** Narrows the shared repository contract to the Rust-backed desktop adapter. */
...
function loadedDocFromSnapshot(snapshot: DocumentSnapshot): LoadedDoc {
  ...
  for (const pageId of snapshot.document.page_ids) {
    ...
    for (const layerId of page.layer_ids) {
      ...
      for (const shapeId of layer.shape_ids) {
        flattenShape(snapshot, page.id, shapeId, undefined, flattened, shapes);
      }
    }
    pages[page.id] = { id: page.id, name: page.name, shapeIds: flattened };
  ...
  for (const binding of Object.values(snapshot.document.bindings)) {
    bindings[binding.id] = {
      ...
      anchor: binding.anchor.kind === "center"
        ? { kind: "center" }
  ...
  return {
    ...
    shapes,
    bindings,
    order: { pageIds: [...snapshot.document.page_ids], shapeOrder },
  };
}
...
function flattenShape(
  ...
  if (shape.kind !== "container") {
    ...
    shapes[shape.id] = legacyShapeFromV2(shape, pageId, groupId);
  }
  for (const childId of shape.child_ids) {
    flattenShape(snapshot, pageId, childId, shape.kind === "container" ? shape.id : groupId, flattened, shapes);
  }
}
...
function applyPatch(doc: LoadedDoc, patch: DocPatch): LoadedDoc {
  ...
  if (patch.order?.shapeOrder) next.order.shapeOrder = {
    ...(next.order.shapeOrder ?? {}),
    ...structuredClone(patch.order.shapeOrder),
  };
  return next;
}

function documentFromLoadedDoc(
  doc: LoadedDoc,
  ...
  for (const pageId of doc.order.pageIds) {
    ...
    if (!page || !currentPage || currentPage.layer_ids.length === 0) {
      throw new Error(`Desktop mirror cannot update unknown page ${pageId}`);
    }
    const layerId = currentPage.layer_ids[0];
    const roots: string[] = [];
    ...
    pages[pageId] = { ...currentPage, name: page.name };
    layers[layerId] = { ...layers[layerId], shape_ids: roots };
  }
...
function shapeFromLegacy(
  ...
  return {
    id: shape.id,
    kind: shape.type,
    parent: shape.groupId
      ? { kind: "shape", id: shape.groupId }
      : { kind: "layer", id: location.layerId },
    transform: {
