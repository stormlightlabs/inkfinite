...
use inkfinite_core::proto::{
    ...
    LayerContentsDisposition, LayerPatch, LayoutAxis, Operation, Proposal, ProposalId,
    ProtocolError, Query, QueryResult, RecordId, Request, Response, SaveResult, SessionId,
    ShapeAlignment, ShapePatch, TransactionDraft, TransactionId, Warning,
};
use inkfinite_core::{
    ActorId, AssetId, AssetRecord, AssetSource, BindingAnchor, BindingId, BindingKind,
    BindingRecord, ChangeHash, ContainerLayout, Document, DocumentId, DocumentSnapshot, FormatId,
    GEOMETRY_BOUNDS, GEOMETRY_COORDINATE_SYSTEM, GEOMETRY_ROTATION, Insets, LayerId, LayerRecord,
    LayoutAlignment, Opacity, Origin, PageId, PageRecord, Provenance, RecordVersion,
    SemanticMetadata, ShapeId, ShapeKind, ShapeParent, ShapeRecord, ShapeStyle, SiblingAnchor,
...
const GENERATED_JSON_COMMENT: &str =
    "Generated by Inkfinite's binding generator. Do not edit this file manually.";
...
fn artifacts() -> Result<BTreeMap<PathBuf, String>, Box<dyn Error>> {
    ...
    artifacts.insert(
        PathBuf::from("fixtures/v2/shape-registry.json"),
        fixture_json()?,
    );
    artifacts.insert(
        PathBuf::from("packages/bindings/src/model.ts"),
        model_bindings(),
    );
    artifacts.insert(
    ...
    artifacts.insert(
        PathBuf::from("packages/bindings/src/protocol.ts"),
        protocol_bindings(),
    );
    artifacts.insert(
        PathBuf::from("packages/bindings/src/registry.ts"),
        registry_bindings(),
    );
    artifacts.insert(
        PathBuf::from("packages/bindings/src/index.ts"),
        index_bindings(),
    );
    Ok(artifacts)
}
...
fn insert_schema<T: JsonSchema>(
    artifacts: &mut BTreeMap<PathBuf, String>,
    relative_path: &str,
    ...
    if let Some(object) = schema.as_object_mut() {
        object.insert(
            "$comment".into(),
            Value::String(GENERATED_JSON_COMMENT.into()),
        );
    }
    artifacts.insert(
...
fn protocol_bindings() -> String {
    let config = ts_config();
    let mut output = GENERATED_TS_HEADER.to_owned();
    output.push_str("import type { ActorId, ChangeHash, DocumentId, DocumentSnapshot, JsonValue, Origin } from \"./model.js\";\n");
    output.push_str(
