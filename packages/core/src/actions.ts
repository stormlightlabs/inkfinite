...
export type WheelAction = {
	...
	/** Point in world coordinates */
	world: Vec2;
	/** Horizontal wheel or trackpad delta in screen pixels */
	deltaX: number;
	/** Vertical wheel or trackpad delta in screen pixels */
	deltaY: number;
	/** Modifier keys state */
	modifiers: Modifiers;
	/** Timestamp of the event */
...
export const Action = {
	 ...
	 */
	wheel(screen: Vec2, world: Vec2, delta: number | Vec2, modifiers: Modifiers, timestamp = Date.now()): WheelAction {
		const deltaX = typeof delta === 'number' ? 0 : delta.x;
		const deltaY = typeof delta === 'number' ? delta : delta.y;
		return { type: 'wheel', screen, world, deltaX, deltaY, modifiers, timestamp };
	},
