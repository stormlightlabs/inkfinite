...
describe('Action', () => {
	...
	describe('wheel', () => {
		it('should create wheel action with all required fields', () => {
			...
			expect(action).toEqual({ type: 'wheel', screen, world, deltaX: 0, deltaY, modifiers, timestamp });
		});

		it('should preserve two-dimensional trackpad deltas', () => {
			const action = Action.wheel(screen, world, { x: 24, y: -48 }, modifiers, timestamp);

			expect(action).toMatchObject({ deltaX: 24, deltaY: -48 });
		});
