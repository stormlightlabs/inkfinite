...
const boards = [
	{ id: 'board:alpha', name: 'Alpha Board', createdAt: 1, updatedAt: 10 },
	{ id: 'board:beta', name: 'Beta Board', createdAt: 2, updatedAt: 20 },
...
describe('FileBrowserVM', () => {
	it('filters boards by query and maintains selection', () => {
		...
		expect(vm.selectedId).toBe('board:gamma');
	...
	it('sorts boards by name when requested', () => {
		const repo = createRepoMock();
