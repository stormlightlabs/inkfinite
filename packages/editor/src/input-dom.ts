...
export class InputAdapter {
	...
	private activeButton: number | null;
	...
	constructor(config: InputAdapterConfig) {
		...
		this.boundHandlers = {
			...
			lostPointerCapture: this.handleLostPointerCapture.bind(this),
	...
	private attach(): void {
		...
		canvas.addEventListener('lostpointercapture', this.boundHandlers.lostPointerCapture);
		...
		window.addEventListener('pointerup', this.boundHandlers.pointerUp, true);
		window.addEventListener('pointercancel', this.boundHandlers.pointerCancel, true);
	...
	dispose(): void {
		...
		canvas.removeEventListener('lostpointercapture', this.boundHandlers.lostPointerCapture);
		...
		window.removeEventListener('pointerup', this.boundHandlers.pointerUp, true);
		window.removeEventListener('pointercancel', this.boundHandlers.pointerCancel, true);
	...
	private handlePointerDown(e: PointerEvent): void {
		...
		try {
			...
			// Window-level release listeners remain as a fallback when capture is
	...
	private finishPointer(e: PointerEvent, forceRelease: boolean): void {
		if (!this.pointerState.isDown || this.activePointerId !== e.pointerId) {
			return;
		}
		...
		const screen = this.getScreenCoords(e);
		const world = this.screenToWorld(screen);
		const buttons = forceRelease ? PointerButtons.create() : PointerButtons.fromButtons(e.buttons);
		const modifiers = Modifiers.fromEvent(e);
		const released = PointerButtons.isEmpty(buttons);
		const button = forceRelease ? (this.activeButton ?? e.button) : e.button;

		this.pointerState.isDown = !released;
		this.pointerState.lastWorld = world;
		this.pointerState.lastScreen = screen;
		this.pointerState.buttons = buttons;
		...
		if (released) {
			try {
				if (this.config.canvas.hasPointerCapture(e.pointerId)) {
