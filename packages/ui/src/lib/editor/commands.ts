...
export function getCommandPaletteEntries(
	...
	const clipEntries: CommandPaletteEntry[] = ['clip-selection', 'remove-clip'].map((id) => ({
		id,
		label: SELECTION_COMMAND_LABELS[id as SelectionCommand],
		group: 'Selection' as const,
		disabled: id === 'clip-selection' ? !canClipSelection(state) : selectedCount === 0,
